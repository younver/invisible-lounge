...
io.on('connection', function(socket){
	...
	socket.on('LOGIN', function (_data)
	{
		...
		console.log('[INFO] player '+currentUser.name+' logged with id '+currentUser.id);
	...
	{
		// data: id, horizontalMovement, jump
	  	var data = JSON.parse(_data);	
		...
		if(currentUser)
		{
			...
			currentUser.horizontal_movement = data.horizontal_movement;
			currentUser.jump = data.jump;

			// send current user position and  rotation in broadcast to all clients in game
			socket.broadcast.emit('UPDATE_MOVE_AND_ROTATE', currentUser.id,currentUser.horizontal_movement,currentUser.jump);
		
			// TODO: Rotation => horMov
		}
	});//END_SOCKET_ON
